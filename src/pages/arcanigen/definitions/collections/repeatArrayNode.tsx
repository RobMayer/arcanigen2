        ...
        <BaseNode<IRepeatArrayNode> nodeId={nodeId} helper={RepeatArrayNodeHelper} hooks={nodeHooks}>
            ...
            <SocketIn<IRepeatArrayNode> nodeId={nodeId} socketId={"iterationCount"} type={SocketTypes.INTEGER}>
                <BaseNode.Input label={"Iteration Count"}>
                    <SliderInput value={iterationCount} onValidValue={setIterationCount} min={1} max={64} step={1} disabled={hasIterationCount} />
                </BaseNode.Input>
            </SocketIn>
            <hr />
...
const Renderer = memo(({ nodeId, depth, globals, overrides }: NodeRendererProps) => {
    ...
    const children = useMemo(() => {
        return lodash.range(iterationCount).map((n, i) => {
            return <g key={i}>{output && childNodeId && <Each overrides={overrides} output={output} host={nodeId} globals={globals} nodeId={childNodeId} depth={depth} index={i} />}</g>;
        });
...
const RepeatArrayNodeHelper: INodeHelper<IRepeatArrayNode> = {
    name: "Repeat Array",
    buttonIcon: nodeIcons.repeatArray.buttonIcon,
    nodeIcon: nodeIcons.repeatArray.nodeIcon,
    flavour: "danger",
