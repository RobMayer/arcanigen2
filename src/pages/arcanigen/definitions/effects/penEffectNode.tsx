        ...
        <BaseNode<IPenEffectNode> nodeId={nodeId} helper={PenEffectNodeHelper} hooks={nodeHooks}>
            ...
            <SocketIn<IPenEffectNode> nodeId={nodeId} socketId={"smudge"} type={SocketTypes.PERCENT}>
                <BaseNode.Input label={"Smudge"}>
                    <NumericInput value={smudge} onValidValue={setSmudge} disabled={hasSmudge} min={0} max={1} />
                </BaseNode.Input>
            </SocketIn>
            <SocketIn<IPenEffectNode> nodeId={nodeId} socketId={"jitter"} type={SocketTypes.PERCENT}>
                <BaseNode.Input label={"Jitter"}>
                    <NumericInput value={jitter} onValidValue={setJitter} disabled={hasJitter} min={0} max={1} />
                </BaseNode.Input>
            </SocketIn>
    ...
    return (
        <>
            <g>
                ...
                <g filter={`url('#effect_${nodeId}_lyr-${depth ?? ""}')`}>{Content && cId && <Content overrides={overrides} globals={globals} nodeId={cId} depth={(depth ?? "") + `_${nodeId}`} />}</g>
            </g>
        </>
    );
});
