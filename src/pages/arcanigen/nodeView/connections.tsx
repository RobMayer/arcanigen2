...
import { LinkType } from "../../../utility/enums";
...
const Connection = ({ linkId, fromNode, toNode, fromSocket, toSocket, type }: ILinkInstance) => {
    ...
    return (
        <ThePath tabIndex={-1} ref={gRef} className={`flavour-${FLAVOUR_BY_TYPE[type]} type-${type}`}>
...
const ThePath = styled.g`
    ...
    &.type-portal > .part_display {
        stroke-width: 6px;
    }
    &.type-portal > .part_special {
    ...
    &:focus.type-portal > .part_special {
        stroke: var(--danger-button);
    }
    &.type-portal > .part_select {
        stroke-width: 16px;
    }
    &:focus > .part_display {
        stroke: var(--theme-link_focushover);
    }
`;
