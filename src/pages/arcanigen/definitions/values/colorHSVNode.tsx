        ...
        <BaseNode<IColorHSVNode> nodeId={nodeId} helper={ColorHSVNodeHelper} hooks={nodeHooks}>
            ...
            <SocketIn<IColorHSVNode> nodeId={nodeId} socketId={"vIn"} type={SocketTypes.PERCENT}>
                <BaseNode.Input label={"Value"}>
                    <SliderInputOld min={0} max={1} value={v} onValidValue={setV} disabled={hasVIn} />
                </BaseNode.Input>
            </SocketIn>
...
const getOutput = (graph: IArcaneGraph, nodeId: string, socket: keyof IColorHSVNode["outputs"], globals: GraphGlobals) => {
