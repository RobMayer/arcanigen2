...
const Controls = memo(({ nodeId, globals }: ControlRendererProps) => {
    ...
    return (
        <BaseNode<ILayersNode> nodeId={nodeId} helper={LayersNodeHelper} hooks={nodeHooks} className={"slim"}>
            ...
            <hr />
            <BaseNode.Input>
                <ActionButton variant={"slim"} onAction={addLayer}>
                    <Icon value={iconActionAdd} /> Add Layer
                </ActionButton>
            </BaseNode.Input>
...
const Renderer = memo(({ nodeId, depth, globals, overrides }: NodeRendererProps) => {
    ...
    return (
        <g>
            {sockets.map((sId, i) => {
                return enabled[sId] ? (
                    <Each key={sId} nodeId={nodeId} socketId={sId} blendMode={modes[sId]} host={nodeId} depth={depth} index={i} globals={globals} overrides={overrides} />
                ) : (
                    <NoOp key={sId} />
                );
            })}
...
const Each = ({
    ...
    return <g style={styles}>{Comp && childId && <Comp nodeId={childId} depth={(depth ?? "") + `_${host}.${index}`} globals={newGlobalsglobals} overrides={overrides} />}</g>;
};

const LayersNodeHelper: INodeHelper<ILayersNode> = {
    name: "Layers",
    buttonIcon: nodeIcons.layers.buttonIcon,
    nodeIcon: nodeIcons.layers.nodeIcon,
    flavour: "danger",
...
const DragTarget = styled(({ index, orderLayer, className, ...props }: HTMLAttributes<HTMLDivElement> & { index: number; orderLayer: (sId: string, pos: number) => void }) => {
