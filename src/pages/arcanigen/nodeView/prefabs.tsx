...
const TransformFull = <T extends INodeDefinition>({ nodeId, hooks }: { nodeId: string; hooks: ReturnType<(typeof ArcaneGraph)["nodeHooks"]> }) => {
    ...
    return (
        <BaseNode.Foldout panelId={"transform"} label={"Transform"} nodeId={nodeId} inputs={"positionX positionY positionTheta positionRadius rotation"} outputs={""}>
            ...
            <SocketIn<T> nodeId={nodeId} socketId={"rotation"} type={SocketTypes.ANGLE}>
                <BaseNode.Input label={"Rotation"}>
                    <RotaryInput value={rotation} onValidValue={setRotation} disabled={hasRotation} />
                </BaseNode.Input>
            </SocketIn>
        </BaseNode.Foldout>
    );
