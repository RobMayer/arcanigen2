...
import { createContext, ForwardedRef, forwardRef, HTMLAttributes, RefObject, useCallback, useContext, useEffect, useImperativeHandle, useMemo, useRef, useState } from "react";
    ...
    forwardRef(({ children, className, ...props }: HTMLAttributes<HTMLDivElement>, fRef: ForwardedRef<DragCanvasControls>) => {
        ...
        const checkBounds = useCallback(() => {
            if (boundsRef.current && outerRef.current) {
                ...
                setIsOutOfBounds((p) => {
                    if (t.borderBottomColor !== p.borderBottomColor || t.borderLeftColor !== p.borderLeftColor || t.borderRightColor !== p.borderRightColor || t.borderTopColor !== p.borderTopColor) {
                        return t;
                    }
                    return p;
                });
        ...
        const setInternalZoom = useCallback(
            (z: number, mouseX: number = 0, mouseY: number = 0) => {
                if (anchorRef.current && panRef.current) {
                    ...
                    const newZoom = Number(Math.min(Math.max(0.1, z), 4).toPrecision(6));
            ...
            if (n && a && p) {
                const handle = (e: WheelEvent) => {
                    if (e.target instanceof HTMLInputElement || e.target instanceof HTMLTextAreaElement) {
                        ...
                        const oldZoom = zoomRef.current;
                        const newZoom = Number(Math.min(Math.max(0.1, oldZoom + e.deltaY * -0.001), 4).toPrecision(6));
...
const Signal = styled.div`
    ...
    inset: 3px;
    pointer-events: none;
    border: 3px solid var(--invalid-button-bg);
    z-index: 99999;
`;
