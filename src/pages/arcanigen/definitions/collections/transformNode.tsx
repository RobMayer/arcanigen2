    ...
    return (
        <BaseNode<ITransformNode> nodeId={nodeId} helper={TransformNodeHelper} hooks={nodeHooks}>
            ...
            <BaseNode.Input label={"Position Mode"}>
                <ToggleList value={positionMode} onSelect={setPositionMode} options={POSITION_MODE_OPTIONS} />
            </BaseNode.Input>
...
const Renderer = memo(({ nodeId, depth, globals, overrides }: NodeRendererProps) => {
    ...
    return (
        <g
            transform={`${MathHelper.getPosition(positionMode, positionX, positionY, positionTheta, positionRadius)} rotate(${postRotation}) scale(${scaleX / 100}, ${
                scaleY / 100
            }) skewX(${skewX}) skewY(${skewY})`}
            vectorEffect={"non-scaling-stroke"}
        >
            <g transform={`rotate(${preRotation})`}>{Output && cid && <Output overrides={overrides} nodeId={cid} depth={`${depth}_${nodeId}`} globals={globals} />}</g>
        </g>
    );
});
...
const TransformNodeHelper: INodeHelper<ITransformNode> = {
    name: "Transform",
    buttonIcon: nodeIcons.transform.buttonIcon,
    nodeIcon: nodeIcons.transform.nodeIcon,
    flavour: "danger",
