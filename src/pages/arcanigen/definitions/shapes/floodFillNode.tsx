        ...
        <BaseNode<IFloodFillNode> nodeId={nodeId} helper={FloodFillNodeHelper} hooks={nodeHooks}>
            ...
            <SocketIn<IFloodFillNode> nodeId={nodeId} socketId={"floodColor"} type={SocketTypes.COLOR}>
                <BaseNode.Input label={"Flood Color"}>
                    <HexColorInput nullable alpha value={floodColor} onValue={setFloodColor} disabled={hasFloodColor} />
                </BaseNode.Input>
            </SocketIn>
...
const Renderer = memo(({ nodeId }: NodeRendererProps) => {
    ...
    return <rect x={"-1000%"} y={"-1000%"} width={"2000%"} height={"2000%"} fill={MathHelper.colorToSVG(floodColor)} fillOpacity={MathHelper.colorToOpacity(floodColor)} />;
});

const FloodFillNodeHelper: INodeHelper<IFloodFillNode> = {
    name: "Flood Fill",
    buttonIcon: nodeIcons.floodFill.buttonIcon,
    nodeIcon: nodeIcons.floodFill.nodeIcon,
    flavour: "emphasis",
