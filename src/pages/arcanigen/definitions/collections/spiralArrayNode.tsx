...
import { PositionMode, SpanMode, SPAN_MODE_OPTIONS, ThetaMode, THETA_MODE_OPTIONS, SpanModes, ThetaModes, NodeTypes, SocketTypes, PositionModes } from "../../../../utility/enums";
    ...
    return (
        <BaseNode<ISpiralArrayNode> nodeId={nodeId} helper={SpiralArrayNodeHelper} hooks={nodeHooks}>
            ...
            <BaseNode.Input label={"Radial Mode"}>
                <ToggleList value={radialMode} onSelect={setRadialMode} options={SPAN_MODE_OPTIONS} />
            </BaseNode.Input>
            ...
            return (
                <g key={n} transform={`rotate(${rot + 180}) translate(0, ${rad})`}>
                    <g key={n} transform={`rotate(${isRotating ? 180 : -rot - 180})`}>
                        {output && childNodeId && <Each overrides={overrides} output={output} nodeId={childNodeId} host={nodeId} depth={depth} globals={globals} index={i} />}
                    </g>
                </g>
            );
        });
    }, [pointCount, thetaMode, thetaStart, thetaEnd, thetaSteps, rI, rO, isRotating, output, childNodeId, thetaInclusive, nodeId, depth, globals, radialCurve, thetaCurve, overrides]);
...
const SpiralArrayNodeHelper: INodeHelper<ISpiralArrayNode> = {
    name: "Spiral Array",
    buttonIcon: nodeIcons.spiralArray.buttonIcon,
    nodeIcon: nodeIcons.spiralArray.nodeIcon,
    flavour: "danger",
